...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2021/10/13 16:41:42 by jofernan          #+#    #+#             */
/*   Updated: 2021/10/15 18:21:13 by jofernan         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
#include "so_long.h"
...
static int	ft_count_buffer_size(char *buffer, t_solong *map)
{
	...
	while (buffer[map->wide] != '\0')
	{
		if (buffer[map->wide] == '\n')
		{
			...
			{
				if ((map->wide - (map->tall - 1))
					- (tomasturba * (map->tall - 1)) != tomasturba)
					return (0);
			}	
			map->tall++;
		}
...
void	map_check(char *argv, t_solong *map)
{
	...
	if (fd < 0)
	{
		printf("ERROR\nNo map file or error while trying to read map file\n");
		return ;
	}
	map->tall = 1;
